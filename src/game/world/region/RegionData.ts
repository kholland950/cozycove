...
export class RegionData {
    ...
    setNoiseMap(type: 'biome' | 'road' | 'elevation' | 'moisture', noiseMap: NoiseMap): void {
        switch (type) {
            case 'biome':
    ...
    getNoiseValue(type: 'biome' | 'road' | 'elevation' | 'moisture', x: number, y: number): number | null {
        ...
        if (!noiseMap || x < 0 || y < 0 || x >= noiseMap.width || y >= noiseMap.height) {
            return null;
        }
